...
export default function SubjectCard({ title, category, description, href, topicsCount, tag }: SubjectCardProps) {
  return (
    <Link
      href={href}
      className="group card-lift block p-6 bg-(--color-paper-100) border border-(--color-concrete-300) rounded-xl hover:border-(--color-teal-700)/60 transition-all duration-300"
    >
      ...
      <p className="text-sm text-(--color-ink-650) mb-4 line-clamp-2 leading-relaxed">
        {description}
      </p>
