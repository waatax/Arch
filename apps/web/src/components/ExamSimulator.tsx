...
export default function ExamSimulator({ catalog }: { catalog: PracticeCatalog }) {
  ...
  if (!session.length) return <section className="rounded-2xl border border-(--color-concrete-300) bg-(--color-paper-100) p-5 shadow-sm sm:p-8" aria-labelledby="setup-title" aria-busy={loading}>
    ...
    <div className="mt-7 grid gap-5 sm:grid-cols-3">
      <label className="text-sm font-bold text-(--color-ink-900)">學年度<select value={year} disabled={loading} onChange={(event) => setYear(Number(event.target.value))} className="mt-2 min-h-12 w-full rounded-lg border border-(--color-concrete-300) bg-(--color-paper-50) px-3 font-normal disabled:cursor-wait disabled:opacity-60">{catalog.years.map(({ year: value }) => <option key={value} value={value}>{value} 年</option>)}</select></label>
      <label className="text-sm font-bold text-(--color-ink-900)">考科<select value={exam} disabled={loading} onChange={(event) => { setExam(event.target.value); setCount('20'); }} className="mt-2 min-h-12 w-full rounded-lg border border-(--color-concrete-300) bg-(--color-paper-50) px-3 font-normal disabled:cursor-wait disabled:opacity-60">{examOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
      <label className="text-sm font-bold text-(--color-ink-900)">題數<select value={count} disabled={loading} onChange={(event) => setCount(event.target.value)} className="mt-2 min-h-12 w-full rounded-lg border border-(--color-concrete-300) bg-(--color-paper-50) px-3 font-normal disabled:cursor-wait disabled:opacity-60"><option value="5">5 題快速練習</option><option value="10">10 題暖身</option><option value="20">20 題練習</option>{exam === 'all' ? <option value="50">50 題五科綜合</option> : null}<option value="all">完整{exam === 'all' ? '五科' : '考科'}（{availableCount} 題）</option></select></label>
    </div>
    ...
    {loadError ? <p role="alert" className="mt-5 rounded-xl border border-(--color-brick-700)/30 bg-(--color-brick-700)/10 px-4 py-3 text-sm font-bold text-(--color-brick-700)">{loadError}</p> : null}
    <button type="button" onClick={start} disabled={loading || availableCount === 0} className="mt-7 min-h-12 rounded-xl bg-(--color-teal-700) px-7 py-3 font-bold text-(--color-paper-50) hover:shadow-lg disabled:cursor-wait disabled:opacity-60">{loading ? '正在載入本回題目…' : '開始整份作答'}</button>
  </section>;
  ...
  return <div className="space-y-6">
    ...
    <ol className="space-y-6">{session.map((question, index) => { const selected = answers[question.id]; const correct = submitted && isAnswerCorrect(question.answer, selected); const multiple = isMultipleChoiceAnswer(question.answer); const startsGroup = Boolean(question.groupId && session[index - 1]?.groupId !== question.groupId); return <Fragment key={question.id}>{startsGroup ? <li className="lesson-deferred-section rounded-2xl border border-(--color-blueprint-700)/30 bg-(--color-paper-50) p-5 sm:p-8"><p className="text-xs font-mono uppercase tracking-widest text-(--color-teal-700)">Passage · 題組 {question.groupId?.split('-g').at(-1)}</p><h2 className="mt-2 font-serif text-xl font-bold leading-8 text-(--color-ink-900)"><MathText content={question.groupTitle} /></h2><div className="mt-5 whitespace-pre-line rounded-xl border-l-4 border-(--color-teal-700) bg-white p-5 text-[15px] leading-8 text-(--color-ink-900) sm:p-7"><MathText content={question.passage} /></div><p className="mt-4 text-xs leading-6 text-(--color-ink-650)">請閱讀上方文章，再依序完成下列所有對應題目。</p></li> : null}<li id={`question-${index + 1}`} className={`lesson-deferred-section scroll-mt-36 rounded-2xl border bg-(--color-paper-100) p-5 sm:p-7 ${submitted ? correct ? 'border-(--color-teal-700)' : 'border-(--color-brick-700)' : 'border-(--color-concrete-300)'}`}>
      ...
      {question.corruptedOcr && (
        <div className="mb-4 rounded-lg border border-(--color-brick-700)/30 bg-(--color-brick-700)/10 p-4 text-sm text-(--color-brick-700) font-bold">
          原題含有複雜數學公式，為避免亂碼，已為您展開官方完整截圖以供作答。
        </div>
      )}
      ...
      {question.figureImage ? <figure className="mt-4 overflow-hidden rounded-xl border border-(--color-concrete-300) bg-white p-2 sm:p-4"><a href={assetHref(question.figureImage)} target="_blank" rel="noreferrer" title="在新分頁放大本題圖面"><img src={assetHref(question.figureImage)} alt={`${question.sourceLabel}第 ${question.questionNo} 題單題圖面`} loading="lazy" decoding="async" className="mx-auto h-auto max-h-[42rem] max-w-full" /></a><figcaption className="mt-2 flex flex-wrap items-center justify-between gap-2 px-1 text-[11px] text-(--color-ink-650)"><span>官方圖表／公式 · 僅裁切原題第 {question.questionNo} 題</span><a href={assetHref(question.figureImage)} target="_blank" rel="noreferrer" className="font-bold text-(--color-teal-700)">放大本題圖面 ↗</a></figcaption></figure> : null}
      <fieldset className="mt-5 grid gap-3 sm:grid-cols-2" disabled={submitted}><legend className={multiple ? 'mb-2 text-sm font-bold text-(--color-ink-650) sm:col-span-2' : 'sr-only'}>{multiple ? `第 ${index + 1} 題為多選題，請選出所有正確選項` : `第 ${index + 1} 題選項`}</legend>{choices.map((choice) => { const active = selected?.includes(choice) ?? false; const answerChoice = submitted && isAnswerChoiceCorrect(question.answer, choice); const wrongChoice = submitted && active && !answerChoice; return <label key={choice} className={`flex min-h-12 cursor-pointer items-start gap-3 rounded-xl border p-4 text-sm leading-6 ${answerChoice ? 'border-(--color-teal-700) bg-(--color-teal-700)/10' : wrongChoice ? 'border-(--color-brick-700) bg-(--color-brick-700)/10' : active ? 'border-(--color-blueprint-700) bg-(--color-paper-50)' : 'border-(--color-concrete-300)'}`}><input type={multiple ? 'checkbox' : 'radio'} name={question.id} value={choice} checked={active} onChange={() => setAnswers((value) => ({ ...value, [question.id]: multiple ? toggleSelectedChoice(value[question.id], choice) : choice }))} className="mt-1" /><strong className="font-mono text-(--color-ink-900)">{choice}</strong>{!question.corruptedOcr && question.options[choice] ? <MathText className="text-(--color-ink-650)" content={question.options[choice]} /> : <span className="text-(--color-ink-650)">原題選項 {choice}</span>}</label>; })}</fieldset>
