...
export default function ExamSimulator({ questions }: { questions: SimulationQuestion[] }) {
  ...
  if (!session.length) return <section className="rounded-2xl border border-(--color-concrete-300) bg-(--color-paper-100) p-5 shadow-sm sm:p-8" aria-labelledby="setup-title">
    ...
    <div className="mt-7 grid gap-5 sm:grid-cols-3">
      ...
      <label className="text-sm font-bold text-(--color-ink-900)">題數<select value={count} onChange={(event) => setCount(event.target.value)} className="mt-2 min-h-12 w-full rounded-lg border border-(--color-concrete-300) bg-(--color-paper-50) px-3 font-normal"><option value="5">5 題快速練習</option><option value="10">10 題暖身</option><option value="20">20 題練習</option>{exam === 'all' ? <option value="50">50 題五科綜合</option> : null}<option value="all">完整{exam === 'all' ? '五科' : '考科'}（{availableCount} 題）</option></select></label>
    </div>
