...
export default function ExamSimulator({ questions }: { questions: SimulationQuestion[] }) {
  ...
  const start = () => {
    ...
    setSession(count === 'all' ? pool : shuffled(pool).slice(0, amount));
    setAnswers({});
    setSubmitted(false);
  ...
  if (!session.length) return <section className="rounded-2xl border border-(--color-concrete-300) bg-(--color-paper-100) p-5 shadow-sm sm:p-8" aria-labelledby="setup-title">
    ...
    <div className="mt-5 rounded-xl bg-(--color-sun-500)/10 px-4 py-3 text-xs leading-6 text-(--color-ink-650)">需要圖表、製圖或版面判讀的題目，會直接顯示測驗中心官方原題圖面，不需另開 PDF。</div>
  ...
  return <div className="space-y-6">
    ...
    <ol className="space-y-6">{session.map((question, index) => { const selected = answers[question.id]; const correct = submitted && isCorrect(question.answer, selected); return <li key={question.id} id={`question-${index + 1}`} className={`scroll-mt-36 rounded-2xl border bg-(--color-paper-100) p-5 sm:p-7 ${submitted ? correct ? 'border-(--color-teal-700)' : 'border-(--color-brick-700)' : 'border-(--color-concrete-300)'}`}>
      ...
      {question.questionImage ? <figure className="mb-5 overflow-hidden rounded-xl border border-(--color-concrete-300) bg-white p-2 sm:p-4"><a href={assetHref(question.questionImage)} target="_blank" rel="noreferrer" title="在新分頁放大原題圖面"><Image src={assetHref(question.questionImage)} alt={`${question.sourceLabel}第 ${question.questionNo} 題官方原題圖面`} width={1092} height={1545} unoptimized className="mx-auto h-auto w-full max-w-4xl" /></a><figcaption className="mt-2 flex flex-wrap items-center justify-between gap-2 px-1 text-[11px] text-(--color-ink-650)"><span>測驗中心官方原題圖面 · 題本第 {question.originalPage} 頁</span><a href={assetHref(question.questionImage)} target="_blank" rel="noreferrer" className="font-bold text-(--color-teal-700)">放大圖面 ↗</a></figcaption></figure> : <h2 className="text-base font-bold leading-8 text-(--color-ink-900)">{question.excerpt}</h2>}
      ...
      {submitted ? <div className="mt-5 border-t border-(--color-concrete-300) pt-5"><p className="font-bold text-(--color-ink-900)">{correct ? `答對；官方答案：${question.answer}` : `答錯；你的答案：${selected}，官方答案：${question.answer}`}</p><p className="mt-2 text-sm text-(--color-ink-650)">考科／考點：{question.subjectName} · {question.topic}</p>{question.lessonRoute ? <a href={localHref(question.lessonRoute)} className="mt-3 inline-flex min-h-11 items-center rounded-lg bg-(--color-teal-700) px-4 text-sm font-bold text-(--color-paper-50)">開啟對應教學 →</a> : <p className="mt-3 text-xs leading-6 text-(--color-ink-650)">共同科目本版提供官方原題圖面與標準答案校對。</p>}</div> : null}
    </li>; })}</ol>
