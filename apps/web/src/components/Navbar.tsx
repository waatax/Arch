...
const navLinks = [
  ...
  { href: '/subjects/mechanics', label: '工程力學' },
  { href: '/subjects/drafting', label: '製圖實習' },
];
    ...
    <nav className="w-full bg-(--color-paper-100)/85 backdrop-blur-md border-b border-(--color-concrete-300) sticky top-0 z-50 transition-all duration-300 hover:shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 group">
          ...
          <span className="text-[10px] bg-(--color-teal-700) text-(--color-paper-50) px-2 py-0.5 rounded-full font-mono tracking-wider">
            v3.0
          </span>
        </Link>
      ...
      >
        <div className="px-4 pb-4 space-y-1 border-t border-(--color-concrete-300)/60">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              onClick={() => setOpen(false)}
              className="block px-4 py-2.5 text-sm font-medium text-(--color-ink-650) hover:text-(--color-teal-700) hover:bg-(--color-teal-700)/5 rounded-lg transition-colors duration-200"
            >
              {link.label}
            </Link>
          ))}
