...
export const autocadData: CadSoftware = {
  ...
  heroMetrics: [
    { label: '全球工程普及率', value: '99% 產業通用標準' },
    { label: '專利檔案格式', value: 'DWG/DXF 跨世代相容' },
    { label: '國家出圖標準', value: 'CNS 11567 100% 吻合' },
    { label: '二次開發語言', value: 'AutoLISP / Visual LISP' },
  ],
  architecturalApplications: [
    {
      ...
      deliverables: ['CNS 標準 A1/A0 建築執照圖集', '1/50 樓梯大樣與外牆斷面圖', '全區水電瓦斯套繪圖說'],
    },
    {
      area: '跨工種外部參考協同套圖',
      ...
      deliverables: ['多工種綜合管線套圖 (CSD/SEM 圖面)', '結構穿越開孔預留圖', '營造工程施工排程干涉圖說'],
    },
    {
      area: '動態圖塊與參數化營建構件庫',
      ...
      deliverables: ['事務所標準動態圖塊構件庫 (.dwg / .dwl)', '自動化門窗五金明細清單', '結構型鋼幾何斷面標準手冊'],
    },
    {
      ...
      deliverables: ['地籍地號套繪分析圖', '現況地形高程整地剖面圖', '都市計畫道路建築線指示圖'],
    },
  ],
  beginnerGuide: {
    introduction: 'AutoCAD 是建築工程師的右手。核心心法是「看指令行、左手鍵盤代碼、右手滑鼠確認、空白鍵隨時 Enter」。永遠保持 1:1 在模型空間畫真實建築尺寸。',
    viewportControls: [
      { action: '平移圖面 (Pan)', keyOrMouse: '按住滑鼠滾輪不放拖曳 (或輸入 P 按空白鍵)', tip: '在無限延伸的模型空間中任意平行游移' },
      { action: '縮放視窗 (Zoom)', keyOrMouse: '滾動滑鼠滾輪 (或輸入 Z 空白鍵後輸入 E)', tip: '滾動滾輪以游標為中心即時放大縮小' },
      { action: '全圖充滿畫面 (Zoom Extents)', keyOrMouse: '滑鼠滾輪快速點擊兩次 (Double Click MMB)', tip: '迷路時最神聖救星，立即將全圖所有物件置中拉滿畫面' },
      { action: '框選方向差異 (Window vs Crossing)', keyOrMouse: '由左往右拉 (藍框實線) vs 由右往左拉 (綠框虛線)', tip: '藍框必須「完全包覆」才選取；綠框「只要碰到邊緣」就全選' },
    ],
    tenStepsSop: [
      { step: 1, title: '設定繪圖單位與精確度', action: '在指令列輸入 UNITS 並按空白鍵', keyPoint: '長度類型設為「十進位 (Decimal)」，精確度設為「0.0」，插入比例設為「公釐 (Millimeters)」' },
      { step: 2, title: '建立 CNS 建築圖層系統', action: '輸入 LA (LAYER) 開啟圖層管理員', keyPoint: '建立 WALL (紅/0.5mm)、BEAM (黃/0.35mm)、DOOR (綠/0.25mm)、TEXT (青/0.25mm)、DIM (灰/0.18mm)' },
      { step: 3, title: '開啟精確物件鎖定 (OSNAP)', action: '按下鍵盤 F3 (切換 OSNAP) 與 F8 (切換正交 Ortho)', keyPoint: '輸入 OSNAP 設定勾選：端點、中點、中心點、交點、垂直點與延伸點' },
      { step: 4, title: '繪製結構柱網基準軸線', action: '輸入 XL (XLINE 構造線) 或 L (LINE) 繪製第一條軸線', keyPoint: '輸入 O (OFFSET) 依序偏移 6000, 7200, 6000 建立柱網網格' },
      { step: 5, title: '放置標準結構柱', action: '輸入 REC (RECTANGLE) 畫 600×600 矩形柱，輸入 H (HATCH) 填入 SOLID 剖面實心色', keyPoint: '全選按 B (BLOCK) 將柱建立為圖塊「COL-600x600」，利用 CO (COPY) 派發至所有軸線交點' },
      { step: 6, title: '繪製雙線外牆與內隔間', action: '輸入 ML (MLINE 複線) 或繪製單線後用 O (OFFSET) 偏移 150mm 或 200mm', keyPoint: '輸入 TR (TRIM) 修剪十字交會處牆體，保持牆體貫通無雜線' },
      { step: 7, title: '切開門窗洞口並插入門扇', action: '在牆面畫輔助線，使用 O 偏移 900mm (門寬)，TR 修剪出洞口', keyPoint: '插入動態門圖塊，利用弧線標明開啟半徑與方向' },
      { step: 8, title: '標註建築尺寸與標高', action: '輸入 DLI (DIMLINEAR 線性標註) 標出開間軸線尺寸', keyPoint: '接著輸入 DCO (DIMCONTINUE 連續標註) 一氣呵成拉完所有連續門窗洞口尺寸' },
      { step: 9, title: '切換至配置圖紙空間 (Layout)', action: '點選左下角「配置 1 (Layout1)」標籤頁', keyPoint: '按右鍵選擇「頁面設定管理員」，設定印表機為 DWG to PDF.pc3，圖紙設為 ISO A1 (841×594 mm)' },
      { step: 10, title: '開視埠並設定 1/100 比例尺出圖', action: '輸入 MV (MVIEW) 拉出矩形視埠，雙擊進入視埠調整視角', keyPoint: '在右下角將比例尺鎖定為「1:100」，載入事務所 monchrome.ctb 輸出完美 PDF' },
    ],
    shortcuts: [
      { key: 'L', command: 'LINE (直線)', explanation: '繪製兩點間之基礎線段', frequency: '必須秒按', mnemonic: 'L 拉線最快' },
      { key: 'PL', command: 'PLINE (聚合線/多段線)', explanation: '繪製具備寬度與連續面積之封閉多段線（算面積必備）', frequency: '必須秒按', mnemonic: 'PL 封閉算面積' },
      { key: 'REC', command: 'RECTANGLE (矩形)', explanation: '指定兩對角點或長寬數值生成矩形框', frequency: '必須秒按', mnemonic: 'REC 框出梁柱' },
      { key: 'C', command: 'CIRCLE (圓形)', explanation: '指定圓心與半徑繪製正圓形', frequency: '高頻常用', mnemonic: 'C 畫圓柱圓洞' },
      { key: 'O', command: 'OFFSET (偏移複製)', explanation: '指定距離將線條平行複製（畫牆厚之生命工具）', frequency: '必須秒按', mnemonic: 'O 偏移長出牆厚' },
      { key: 'TR', command: 'TRIM (修剪)', explanation: '點擊交線自動剪除多餘突出線段；按住 Shift 切換為延伸 (EXTEND)', frequency: '必須秒按', mnemonic: 'TR 剪去雜線、Shift 延伸', contextModifier: '按住 Shift 鍵直接切換為延伸 (Extend)' },
      { key: 'EX', command: 'EXTEND (延伸)', explanation: '將選定線條精確延伸至特定邊界目標', frequency: '高頻常用', mnemonic: 'EX 延伸碰頭' },
      { key: 'M', command: 'MOVE (移動)', explanation: '選取基準點將物件平移至目標座標點', frequency: '必須秒按', mnemonic: 'M 移動位置' },
      { key: 'CO / CP', command: 'COPY (複製)', explanation: '多重複製圖元，輸入精確間距可連打陣列', frequency: '必須秒按', mnemonic: 'CO 複製不斷' },
      { key: 'RO', command: 'ROTATE (旋轉)', explanation: '選取旋轉中心點，輸入角度或利用 R 參考角度精準旋轉', frequency: '必須秒按', mnemonic: 'RO 旋轉角度', contextModifier: '輸入 R 可進行已知斜邊角度參考旋轉' },
      { key: 'F', command: 'FILLET (圓角/倒角)', explanation: '將半徑設為 0 (R=0) 時，兩條不平行的折線瞬間精準修剪成尖角', frequency: '必須秒按', mnemonic: 'F 圓角、R=0 尖角收邊' },
      { key: 'MA', command: 'MATCHPROP (性質複製)', explanation: '營造界刷子神鍵，一鍵將目標物件圖層、顏色、線型同步', frequency: '必須秒按', mnemonic: 'MA 屬性刷複製' },
    ],
    fatalTraps: [
      {
    ...
    proTips: [
      { title: '多善用 F8 (正交) 與 F10 (極座標追蹤) 快速切換', description: '繪製建築開間正交線時常駐 F8；遇到 45 度或 30/60 度斜切屋頂時切換為 F10，極速捕捉特定斜交射線。' },
      { title: 'F 圓角指令 R=0 是最神速的兩線修剪收頭法', description: '兩條線沒有相交或互相穿透時，不用按 Trim 慢慢剪；直接按 F 空白鍵 (確保 Radius=0) 點兩條線，系統自動以光速將兩端修剪閉合為完美轉角。' },
      { title: '文字遮罩 (WIPEOUT / 標註背景遮罩) 拯救繁雜圖面', description: '尺寸標註或文字穿越柱梁剖面線時，切勿手動剪斷剖面線！在標註性質中勾選「填入顏色 → 背景」，標註文字會自動壓住背景線條，保持底圖完整。' },
    ],
  },
  sevenIterations: [
    {
      ...
      advancedParameters: [
        { name: 'OSMODE (鎖定模式旗標碼)', value: '16383 (開啟全功能鎖定) 或 4287 (建築推薦設定)', purpose: '精準二進制控制端點、交點與垂直點捕捉' },
        { name: 'ANGBASE (角度基準方向)', value: '0.0 (東向 3 點鐘方向為 0 度)', purpose: '符合標準數學角度定義' },
        { name: 'ANGDIR (角度旋轉方向)', value: '0 (逆時針為正角度)', purpose: '統一全事務所旋轉計算標準' },
      ],
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'LTSCALE (全域線型比例)', value: '1.0 (模型空間) 搭配 PSLTSCALE=1', purpose: '確保虛線與中心線在任何比例視埠中長度一致' },
        { name: 'CELTSCALE (目前物件線型比例)', value: '1.0 (禁止隨意更改個別圖元)', purpose: '維護圖面線型風格全案統一' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Visibility States (可見性狀態)', value: '單開門 / 雙開門 / 子母門 / 推拉門', purpose: '單一圖塊集合整併同類構件' },
        { name: 'Linear Parameter - Number of Grips', value: '1 (單側握柄拉伸)', purpose: '固定門鉸鏈端點僅允許門片拉伸' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'HPGAPTOL (剖面線間隙容差)', value: '1.0 mm (容許微小公差閉合)', purpose: '避免微小開口導致無法生成填充線' },
        { name: 'HPORIGIN (剖面線原點定位)', value: '手動指定磁磚交點起算', purpose: '精確檢討地壁磚施工起磚點與計畫縫' },
      ],
      practicalWalkthrough: [
        '繪製 RC 外牆與鋁門窗崁縫大樣輪廓',
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'XREFTYPE (外部參考預設類型)', value: '1 (Overlay 覆疊參照)', purpose: '徹底根絕嵌套參照引發的循環死結' },
        { name: 'XLOADCTL (隨選載入控制)', value: '2 (開啟隨選載入並複製檔案)', purpose: '允許多人同時唯讀檢視同一個外參檔案' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'FLATTEN (壓平指令容差)', value: '隱藏 3D 實體保留正交 2D 投影', purpose: '消除所有圖元殘留的微小 Z 座標' },
        { name: 'EXPORTTOAUTOCAD 版本', value: 'AutoCAD 2018/2013 DWG 格式', purpose: '向下相容公部門與營造廠老舊審查機台' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'ACADLSPASDOC (LISP 載入行為)', value: '1 (每個開啟的圖檔皆自動載入 acaddoc.lsp)', purpose: '常駐事務所專屬客製化功能巨集' },
        { name: 'FIELDDISPLAY (欄位背景灰底)', value: '0 (關閉列印灰底顯示)', purpose: '保持圖紙輸出乾淨清爽' },
      ],
      practicalWalkthrough: [
  ...
  industryPipeline: {
    ...
    fileFormats: {
      import: ['.dwg', '.dxf', '.dgn', '.sat', '.pdf', '.tif', '.jpg', '.png'],
      export: ['.dwg', '.dxf', '.dwf', '.pdf', '.wmf', '.sat', '.eps'],
    },
    collaborationWith: ['Revit (匯入底圖或匯出施工大樣)', 'SketchUp (匯入 CAD 平面放樣)', '3ds Max (匯入 DWG 樣條線建模)', 'Excel (雙向動態資料連結表格)'],
  },
};
