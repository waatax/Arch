...
export const threeDsMaxData: CadSoftware = {
  ...
  heroMetrics: [
    { label: '視覺化效果圖市占', value: '全球 ArchViz 90% 份額' },
    { label: '外掛生態系成熟度', value: 'ForestPack / RailClone 標配' },
    { label: '渲染光影物理擬真', value: 'Corona / V-Ray 雙核天花板' },
    { label: '材質光學真實度', value: 'PBR / ACEScg 完美色域' },
  ],
  architecturalApplications: [
    {
      ...
      deliverables: ['8K 競圖主視覺透視大片', '夜景照明工程渲染效果圖', '鳥瞰全區總體規劃立體視覺圖'],
    },
    {
      ...
      deliverables: ['高擬真森林景觀效果圖', '四季植被色彩季相變換模擬圖', '景觀植物配置數量概算表'],
    },
    {
      ...
      deliverables: ['全參數化曲線幕牆系統', '超長公共工程天棚構造模型', '施工裝修細部節點立體大樣'],
    },
    {
      ...
      deliverables: ['無拉伸 1:1 施工紋理對照模型', '歷史建築風化水漬材質貼圖集', '近景構造節點寫實特寫透視'],
    },
  ],
  beginnerGuide: {
    introduction: '3ds Max 的靈魂在於右側「命令面板 (Command Panel)」與「修改器堆疊 (Modifier Stack)」。永遠以非破壞性思考建模，每一個修改步驟都可隨時回溯調參。',
    viewportControls: [
      { action: '視圖旋轉 (Orbit)', keyOrMouse: 'Alt + 按住滑鼠滾輪不放拖曳', tip: '以當前選取物件為中心平滑環繞視角' },
      { action: '視圖平移 (Pan)', keyOrMouse: '按住滑鼠滾輪不放拖曳', tip: '平移當前工作視圖' },
      { action: '平滑縮放 (Zoom)', keyOrMouse: 'Ctrl + Alt + 按住滑鼠滾輪不放拖曳 (比單純滾動滾輪更細膩)', tip: '極度微距推進模型觀察接縫' },
      { action: '最大化當前視埠', keyOrMouse: 'Alt + W', tip: '在四視圖 (頂/前/左/透視) 與單一全螢幕視圖之間極速切換' },
    ],
    tenStepsSop: [
      { step: 1, title: '嚴格設定自訂單位 (Units Setup)', action: '進入 Customize → Units Setup，點擊「System Unit Setup」', keyPoint: '系統單位 (System Unit) 必須設為 1 Unit = 1.0 Millimeters (公釐)，顯示單位也設為 Metric - Millimeters' },
      { step: 2, title: '匯入建築 CAD 底圖', action: '點選 File → Import，選取已清理過的建築 DWG 檔案', keyPoint: '在匯入設定勾選「Rescale」單位選 Millimeters，勾選「Weld nearby vertices」融合端點' },
      { step: 3, title: '凍結底圖防止誤觸', action: '選中底圖按滑鼠右鍵 → Object Properties，取消勾選「Show Frozen in Gray」', keyPoint: '按右鍵選擇「Freeze Selection」，底圖保留原色且不再會被滑鼠意外選中' },
      { step: 4, title: '開啟 2.5D 鎖定描繪牆體輪廓', action: '按鍵盤 S 開啟鎖定，右鍵長按鎖定圖示切換為「2.5D Snap」', keyPoint: '利用 Spline → Line 沿著底圖牆體閉合描繪，2.5D 能防止滑鼠飛到空中 Z 軸' },
      { step: 5, title: '添加 Extrude 修改器擠出牆高', action: '選中描繪好的封閉樣條線，在 Modifier List 選擇「Extrude」', keyPoint: '在 Amount 輸入 3600 mm，勾選「Cap Start」與「Cap End」封閉上下頂面' },
      { step: 6, title: '轉為 Edit Poly 進行門窗切削', action: '在修改器堆疊最上方按右鍵 → Convert to Editable Poly', keyPoint: '切換至 Edge (邊) 模式按 Connect 加上下切線，選中面按 Bridge 挖穿開洞' },
      { step: 7, title: '架設物理相機 (Physical Camera)', action: '切換至頂視圖，點擊 Cameras → Standard → Physical Camera', keyPoint: '從室內拉向窗外，按 C 鍵切換至相機視角，勾選「Tilt Correction」自動垂直立柱' },
      { step: 8, title: '指定太陽與物理天空 (Sun & Sky)', action: '在 Lights 面板建立 CoronaSun 或 V-Ray Sun，點擊時系統詢問是否新增 Sky 環境', keyPoint: '點擊「Yes」，將太陽仰角拉高至 45 度，光影即時投射進室內地坪' },
      { step: 9, title: '指派 PBR 物理建築材料', action: '按鍵盤 M 打開板岩材質編輯器 (Slate Material Editor)', keyPoint: '建立 CoronaPhysicalMtl，連接 Diffuse 貼圖與 Normal 貼圖，拖曳賦予建築牆體' },
      { step: 10, title: '啟動互動式即時算圖 (Interactive Render)', action: '在 VFB (Virtual Frame Buffer) 視窗點擊「Start Interactive」', keyPoint: '視窗中光影即時以每秒 30 幀運算，隨意移動家具燈光，光斑倒影即刻響應' },
    ],
    shortcuts: [
      { key: 'Q', command: '選取模式 (Select Tool)', explanation: '安全選取鍵，防止手滑誤移動構件', frequency: '必須秒按', mnemonic: 'Q 純選不亂動' },
      { key: 'W', command: '移動變換 (Move)', explanation: '啟用空間三軸移動坐標軸心 (Gizmo)', frequency: '必須秒按', mnemonic: 'W 移動空間' },
      { key: 'E', command: '旋轉變換 (Rotate)', explanation: '啟用三維球形旋轉軸心', frequency: '必須秒按', mnemonic: 'E 旋轉轉向' },
      { key: 'R', command: '等比/非等比縮放 (Scale)', explanation: '啟用縮放變換軸心', frequency: '必須秒按', mnemonic: 'R 縮放比例' },
      { key: 'M', command: '材質編輯器 (Material Editor)', explanation: '打開板岩材質編輯器配置所有著色器節點', frequency: '必須秒按', mnemonic: 'M 材質大本營' },
      { key: 'C', command: '相機視角 (Camera View)', explanation: '直接將當前視埠切換為場景中所架設的相機鏡頭視野', frequency: '必須秒按', mnemonic: 'C 鏡頭對焦點' },
      { key: 'P', command: '透視視圖 (Perspective View)', explanation: '退出相機視圖切回自由透視觀察模式', frequency: '必須秒按', mnemonic: 'P 自由看全場' },
      { key: 'Alt + W', command: '最大化視埠 (Maximize Viewport)', explanation: '將當前選定的視窗切換為全螢幕工作狀態', frequency: '必須秒按', mnemonic: 'Alt+W 全螢幕切換' },
      { key: 'F3', command: '線框/實體著色切換 (Wireframe Toggle)', explanation: '在透視線框與 PBR 著色外觀之間快速切換', frequency: '必須秒按', mnemonic: 'F3 穿透看骨架' },
      { key: 'F4', command: '顯示幾何邊面 (Edged Faces)', explanation: '在著色表面上疊加顯示多邊形拓撲邊線（建模必備常駐）', frequency: '必須秒按', mnemonic: 'F4 邊線全現形' },
      { key: 'Shift + Q', command: '快速渲染 (Quick Render)', explanation: '立即以當前設定啟動最終影格彩現', frequency: '高頻常用', mnemonic: 'Shift+Q 快速出大圖' },
      { key: 'G', command: '隱藏/顯示工作格線 (Grid Toggle)', explanation: '隱藏視窗中干擾視覺的灰色座標格線', frequency: '高頻常用', mnemonic: 'G 格線開與關' },
    ],
    fatalTraps: [
      {
    ...
    proTips: [
      { title: '2.5D 捕捉 (2.5D Snap) 是建築描圖之王', description: '在透視或正交視圖描繪 CAD 底圖時，務必長按鎖定圖示切換為「2.5D」。它能捕捉空間中具有高差的 CAD 頂點，但將畫出的樣條線全部壓平在當前工作平面，絕不產生 Z 軸亂跳！' },
      { title: 'LightMix 讓你算一張圖搞定白晝、黃昏與夜景', description: '在 Corona 算圖前點擊「Setup LightMix」，算完圖後不用重新渲染，只需拖動滑條關閉太陽、調亮室內筒燈並將色溫改為 2700K 暖光，3 秒切換為奢華夜景。' },
      { title: 'Chamfer 修改器取代真實高面數倒角', description: '不要手動在幾何體上切無數細分圓角；在物體最上方掛一個 Chamfer 修改器，設定 Amount 3mm，勾選 Smooth 只平滑倒角，既能反射極致金屬高光又極度輕量。' },
    ],
  },
  sevenIterations: [
    {
      ...
      advancedParameters: [
        { name: 'System Unit Scale', value: '1.0 Millimeter (禁止使用預設 Inches)', purpose: '保證光學物理衰減與置換高度 1:1 準確' },
        { name: 'Display Driver', value: 'Nitrous Direct3D 11 (Advanced Performance)', purpose: '提供數百萬面即時視埠陰影與環境遮蔽' },
      ],
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Interpolation Steps', value: 'Steps: 6 (正交) / Adaptive (自由弧線)', purpose: '兼顧曲線滑順度與多邊形面數優化' },
        { name: 'Sweep Pivot Alignment', value: '9 點對齊網格精確選擇內外牆錨點', purpose: '防止線板內嵌至牆體結構內部' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Turbosmooth Iterations', value: 'Viewport: 1 / Render: 3', purpose: '保持視埠操作流暢同時確保算圖極致光滑' },
        { name: 'Crease Edge Weight', value: '0.0 (平滑) 至 1.0 (絕對銳利)', purpose: '精確控制帷幕鋁板摺邊硬度' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Texel Density (紋素密度)', value: '10.24 px/cm (保證 4K 算圖近景不模糊)', purpose: '全場景模型材質解析度統一標準化' },
        { name: 'Normal Map Flip Green (Y)', value: '根據渲染器切換 DirectX (-Y) 或 OpenGL (+Y)', purpose: '避免磚牆凹凸反向變成突起' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Corona Pass Limit', value: '150 - 250 passes (或 Noise Threshold: 3%)', purpose: '平衡算圖時間與無噪點畫面純淨度' },
        { name: 'Color Space', value: 'ACEScg (寬色域色彩管理)', purpose: '消除強烈陽光直射處的高光死白泛黃' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Datasmith Import Filter', value: 'Geometry + Materials + Lights', purpose: '無損同步建築模型結構' },
        { name: 'Mesh Weld Threshold', value: '0.1 mm', purpose: '自動縫合 BIM 構件微小縫隙消除漏光' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Batch Render Output', value: '32-bit OpenEXR (Multichannel: RGB, Alpha, Depth, Normal, Cryptomatte)', purpose: '提供合成後期極致無損動態範圍' },
        { name: 'ControlNet Depth Weight', value: '1.0 (絕對幾何約束鎖定)', purpose: '確保 AI 生成風格時建築梁柱形體絲毫不形變' },
      ],
      practicalWalkthrough: [
  ...
  industryPipeline: {
    ...
    fileFormats: {
      import: ['.max', '.3ds', '.dwg', '.dxf', '.fbx', '.obj', '.sat', '.skp', '.abc', '.step'],
      export: ['.max', '.fbx', '.obj', '.abc (Alembic)', '.datasmith', '.exr', '.png', '.tif'],
    },
    collaborationWith: ['AutoCAD (匯入線條放樣)', 'Revit (經由 FBX/Datasmith 匯入建築)', 'Photoshop (EXR 多通道合成調光)', 'Unreal Engine 5 (即時光追遊戲級漫遊)'],
  },
};
