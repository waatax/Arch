...
export const revitData: CadSoftware = {
  ...
  heroMetrics: [
    { label: '全球 BIM 標案市占率', value: 'No.1 旗艦標準' },
    { label: '平立剖關聯同調性', value: '100% 雙向即時連動' },
    { label: '參數化程式設計', value: 'Dynamo 內建引擎' },
    { label: '多專業整合涵蓋', value: 'Arch / Str / MEP 全覆蓋' },
  ],
  architecturalApplications: [
    {
      ...
      deliverables: ['高精度 LOD 350 建築施工模型', '法定樓地板與容積檢討明細表', '自動化平立剖面施工圖紙集'],
    },
    {
      ...
      deliverables: ['事務所標準參數化門窗族群庫 (.rfa)', '自適應帷幕單元樣板', '智慧型電梯與逃生梯核心筒族群'],
    },
    {
      ...
      deliverables: ['雲端中央整合專案檔 (.rvt)', '協同衝突歷史審查紀錄表', '工種模型劃分拆分架構書'],
    },
    {
      ...
      deliverables: ['CSD 綜合機電圖說 (Combined Services Drawing)', 'SEM 結構留孔套圖 (Structural & Electrical / Mechanical)', 'BCF 衝突議題指派報告書'],
    },
  ],
  beginnerGuide: {
    introduction: '學習 Revit 必須先忘記 AutoCAD 的「手動畫線」思維！在 Revit 裡你不是在繪圖，而是在電腦裡「蓋房子」。你放置的是有厚度、有材料、有名稱的「實體建築構件」。',
    viewportControls: [
      { action: '平移視圖 (Pan)', keyOrMouse: '按住滑鼠滾輪不放並拖曳', tip: '平移任何 2D 平面或 3D 視圖' },
      { action: '3D 視角旋轉 (Orbit)', keyOrMouse: 'Shift + 按住滑鼠滾輪不放拖曳', tip: '圍繞當前選取物件或模型幾何中心旋轉視角' },
      { action: '快速縮放至充滿視窗', keyOrMouse: '鍵盤連按兩次 Z + A (Zoom All) 或滑鼠雙擊滾輪', tip: '一鍵讓視圖內所有模型元件置中滿版' },
      { action: '定向至視圖 (Orient to View)', keyOrMouse: '在 3D ViewCube 上按右鍵 → 轉向視圖 → 樓層平面', tip: '在 3D 模式下一鍵切成該樓層的 3D 剖切盒透視！極具震撼感' },
    ],
    tenStepsSop: [
      { step: 1, title: '設定專案單位與標註格式', action: '在鍵盤直接輸入 UN (UNITS)', keyPoint: '長度改為「公釐 (Millimeters)」，面積改為「平方公尺 (m²)」，捨入取至小數點後 2 位' },
      { step: 2, title: '建立建築標高 (Levels)', action: '在專案瀏覽器雙擊開啟「南向立面圖 (South Elevation)」', keyPoint: '輸入 LL (LEVEL)，建立 1F (0 mm), 2F (3600 mm), 3F (7200 mm), RF (10800 mm)' },
      { step: 3, title: '建立結構柱網軸線 (Grids)', action: '雙擊開啟「1F 平面視圖」，輸入 GR (GRID)', keyPoint: '依序繪製 X 軸 (1, 2, 3...) 與 Y 軸 (A, B, C...)，軸線交點即為結構落柱基準' },
      { step: 4, title: '放置結構柱 (Structural Columns)', action: '切換至「結構」標籤頁，點選「柱」或輸入 CL', keyPoint: '在選項列確認深度改為「高度」，頂部約束設定為「2F」，直接放置於軸網交點' },
      { step: 5, title: '繪製複合外牆 (Walls)', action: '切換至「建築」標籤頁，輸入 WA (WALL)', keyPoint: '選擇「RC 200mm + 外牆粉刷」，定位線設為「核心面: 外部」，底部 1F、頂部約束至 2F' },
      { step: 6, title: '放置建築地坪樓板 (Floors)', action: '輸入 SB 或點選「樓板 (Floor)」，利用拾取牆體 (Pick Walls) 圍閉輪廓', keyPoint: '完成編輯打綠色勾勾，系統會貼心詢問是否將牆延伸至樓板底' },
      { step: 7, title: '插入門窗構件 (Doors & Windows)', action: '輸入 DR (Door) 或 WN (Window)，滑鼠直接靠近牆體', keyPoint: '門窗會自動吸附於牆體並自動挖穿牆身，按空白鍵可切換開啟方向與內外側' },
      { step: 8, title: '繪製樓梯與天井洞口 (Stairs)', action: '點選「樓梯 (Stair)」，設定底部 1F、頂部 2F，系統自動計算級高與級深', keyPoint: '使用「豎板工具 (Shaft Opening)」一次穿透所有樓層樓板開出樓梯間天井' },
      { step: 9, title: '標註房間名稱與計算面積 (Rooms)', action: '輸入 RM (ROOM) 點選每個房間，自動識別牆體邊界', keyPoint: '給予房間名稱「客廳」、「主臥」、「陽台」，立即產出全自動著色分區圖' },
      { step: 10, title: '拖曳至圖紙並出圖 (Sheets)', action: '專案瀏覽器在「圖紙 (Sheets)」按右鍵新建 A1 圖紙', keyPoint: '將 1F 平面圖從左側瀏覽器直接拖入圖紙，設定 1/100 比例，雙向關聯施工圖大功告成' },
    ],
    shortcuts: [
      { key: 'WA', command: 'WALL (牆體)', explanation: '啟動繪製建築或結構牆體命令', frequency: '必須秒按', mnemonic: 'WA 牆體圍空間' },
      { key: 'DR', command: 'DOOR (門)', explanation: '在任何現有牆體上插入門族群並自動開洞', frequency: '必須秒按', mnemonic: 'DR 門扉開啟' },
      { key: 'WN', command: 'WINDOW (窗)', explanation: '在牆體上插入鋁門窗或帷幕天窗', frequency: '必須秒按', mnemonic: 'WN 窗明几淨' },
      { key: 'CL', command: 'COLUMN (柱)', explanation: '放置建築裝修柱或承重結構柱', frequency: '必須秒按', mnemonic: 'CL 柱立天地' },
      { key: 'CM', command: 'COMPONENT (元件/族群)', explanation: '放置家具、衛生設備、機電五金等各種三維族群', frequency: '必須秒按', mnemonic: 'CM 元件隨手放' },
      { key: 'AL', command: 'ALIGN (對齊)', explanation: '將一個物件的邊緣與另一個物件完美平齊，並可上一把鎖永久固定', frequency: '必須秒按', mnemonic: 'AL 對齊上鎖', contextModifier: '按對齊後點擊藍色鎖頭可鎖定間距' },
      { key: 'TR', command: 'TRIM / EXTEND (修剪/延伸至角)', explanation: '點選想要保留的兩道牆，自動交會修剪收邊（跟 CAD 點多餘線不同！）', frequency: '必須秒按', mnemonic: 'TR 點保留、自動接角' },
      { key: 'SL', command: 'SPLIT ELEMENT (分割圖元)', explanation: '將一道長牆在某點切斷為兩段獨立牆體', frequency: '高頻常用', mnemonic: 'SL 一刀兩斷' },
      { key: 'CS', command: 'CREATE SIMILAR (建立類似物件)', explanation: '點選畫面上任何柱子或窗戶按 CS，立即進入繪製同類型物件命令', frequency: '必須秒按', mnemonic: 'CS 複製屬性立刻畫' },
      { key: 'BX', command: 'SELECTION BOX (框選剖切盒)', explanation: '在 2D 平面選中某個複雜節點按 BX，3D 視角瞬間自動剖切出該細部！', frequency: '必須秒按', mnemonic: 'BX 3D 局部剖切' },
      { key: 'VV / VG', command: 'VISIBILITY / GRAPHICS (可見性/圖形覆寫)', explanation: '控制當前視圖中所有建築分類的顯示、隱藏、線條顏色與填充樣式', frequency: '必須秒按', mnemonic: 'VV 控制圖元外觀' },
      { key: 'UN', command: 'PROJECT UNITS (專案單位)', explanation: '隨時切換專案為公釐、公尺或英制單位', frequency: '高頻常用', mnemonic: 'UN 單位自由轉' },
    ],
    fatalTraps: [
      {
    ...
    proTips: [
      { title: '善用暫時隱藏/隔離快捷鍵 HH 與 HR', description: '選取擋住視線的屋頂或外牆，按兩次 H (HH) 立即暫時隱藏；按 HR (Reset) 立即解除隔離恢復原貌，排查內部管線超級靈活。' },
      { title: '鍵盤空白鍵 (Spacebar) 是族群旋轉吸附神器', description: '放置門、窗、家具或衛生設備時，按空白鍵可旋轉 90 度；若游標靠近一條斜向牆體按空白鍵，物件會自動旋轉至與該斜牆完全平行！' },
      { title: '在立面圖中對齊上鎖可實現全棟窗戶同步調高', description: '在外牆立面用 AL (Align) 對齊窗台高度並點下「藍色掛鎖」，未來只要修改其中一扇窗的高度，全棟該立面所有窗台立即連鎖同步。' },
    ],
  },
  sevenIterations: [
    {
      ...
      advancedParameters: [
        { name: 'Detail Level (詳細程度)', value: '粗略 (Coarse) / 中等 (Medium) / 精細 (Fine)', purpose: '控制複合構造剖切時顯示為單色或各層材料' },
        { name: 'Model In-Place (現地模型)', value: '僅限異形造型，禁止大量濫用', purpose: '維持資料庫快取效能與明細表關聯性' },
      ],
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Core Boundary (核心邊界)', value: '將 RC 結構層包裹於核心層內部', purpose: '定義結構尺寸標註與樓板承托基準線' },
        { name: 'Wrapping at Ends (端點收頭)', value: 'Exterior (由外層面材包覆端頭)', purpose: '精確模擬開口與外牆轉角磁磚收頭' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'IsReference (參考性質)', value: 'Strong Reference (優先捕捉對齊)', purpose: '使專案中標註尺寸時能瞬間吸附到族群中心線' },
        { name: 'Shared Parameter (共用參數)', value: 'GUID 全域綁定', purpose: '允許族群內部參數被載入明細表進行工料分析' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Vertical Grid Layout', value: 'Fixed Distance: 1500 mm', purpose: '符合標準玻璃原板經濟裁切模矩' },
        { name: 'Horizontal Grid Layout', value: 'Fixed Number: 依樓層均分', purpose: '對齊各樓層梁底結構淨高' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Phase Filter', value: 'Show Previous + New (顯示既有與新建)', purpose: '標準室內裝修與舊屋翻新圖說' },
        { name: 'Material: As Area', value: '計算扣除門窗洞口之淨牆面泥作面積', purpose: '精確工程預算估驗' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'IFC Export MVD', value: 'IFC4 Reference View 1.2', purpose: '國際公共工程 OpenBIM 審查合規版本' },
        { name: 'COBie Extension Setup', value: '空間分類與資產設備序號映射', purpose: '銜接營運維護階段 CAFM 系統' },
      ],
      practicalWalkthrough: [
    ...
    },
    {
      ...
      advancedParameters: [
        { name: 'Dynamo Run Type', value: 'Manual (手動執行，嚴禁在百萬面模型設為 Automatic)', purpose: '防止隨意連線引發背景死鎖崩潰' },
        { name: 'Revit TransactionGroup', value: 'Assimilation (融合多重步驟為單一步驟)', purpose: '維持 Undo 歷程乾淨清晰' },
      ],
      practicalWalkthrough: [
  ...
  industryPipeline: {
    ...
    fileFormats: {
      import: ['.rvt', '.rfa', '.ifc', '.dwg', '.dxf', '.dgn', '.skp', '.sat', '.nwd', '.pdf'],
      export: ['.rvt', '.ifc', '.dwg', '.dxf', '.nwc', '.fbx', '.gbxml', '.pdf', '.txt (明細表)'],
    },
    collaborationWith: ['AutoCAD (雙向匯入出圖大樣)', 'Navisworks (碰撞衝突與 4D 時程檢討)', 'Rhino/Grasshopper (經由 Rhino.Inside.Revit 無縫對接)', 'Twinmotion/Enscape (即時渲染)'],
  },
};
